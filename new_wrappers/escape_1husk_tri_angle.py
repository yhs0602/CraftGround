...
class Escape1HuskTriAngleWrapper(gym.Wrapper):
    ...
    def reset(self, fast_reset: bool = True) -> WrapperObsType:
        ...
        obs = obs["obs"]
        sound_subtitles = obs.sound_subtitles
        sound_vector = self.encode_sound_and_yaw(sound_subtitles, obs.x, obs.y, obs.z)
        return np.array(sound_vector, dtype=np.float32)


def main():
    env = Escape1HuskTriAngleWrapper(verbose=False)
    buffer_size = 1000000
    batch_size = 256
    gamma = 0.99
