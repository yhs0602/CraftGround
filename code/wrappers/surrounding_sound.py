...
# Sound wrapper
class SurroundingSoundWrapper(CleanUpFastResetWrapper):
    def __init__(self, env, sound_list, coord_dim):
        ...
        self.observation_space = gym.spaces.Box(
            low=-1, high=1, shape=(len(sound_list) * coord_dim + 2,), dtype=np.float32
        )
    ...
    def encode_sound(
        ...
        sound_vector = [0] * (len(self.sound_list) * self.coord_dim + 2)
