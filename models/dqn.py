...
class CNNDQNStacked(nn.Module):
    def __init__(self, input_shape, num_actions, hidden_dim):
        super(CNNDQNStacked, self).__init__()
        # input shape: (4, 3, 114, 64) (N, C, H, W)
        self.conv1 = nn.Conv3d(
            input_shape[1], 32, kernel_size=(3, 3, 3), padding=1
        )  # (3, 210, 160)
        ...
        self.fc1 = nn.Linear(128 * 4 * 4 * 4, 256, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, num_actions)

    def forward(self, x):
        x = x.float() / 255.0
        x = x.transpose(1, 3).transpose(1, 2)  # (4, 3, 114, 64) to (4, 114, 64, 3)
        print(f"before 1 :{x.shape}")
        x = self.pool1(nn.functional.relu(self.conv1(x)))
        print(f"before 2 :{x.shape}")
        x = self.pool2(nn.functional.relu(self.conv2(x)))
        print(f"before 3 :{x.shape}")
...
class CNNDQNWithBN(nn.Module):
    def __init__(self, input_shape, num_actions, kernel_size, stride, hidden_dim):
        super(CNNDQNWithBN, self).__init__()
        self.conv1 = nn.Conv2d(
            input_shape[0], 16, kernel_size=kernel_size, stride=stride
        )  # (210, 160, 3), permuted to (3, 210, 160)
        self.conv2 = nn.Conv2d(16, 32, kernel_size=kernel_size, stride=stride)
    ...
    def forward(self, x):
        ...
        x = nn.functional.relu(self.conv1(x))
        x = nn.functional.relu(self.conv2(x))
        x = nn.functional.relu(self.conv3(x))  # self.bn3(
        x = x.view(x.size(0), -1)
