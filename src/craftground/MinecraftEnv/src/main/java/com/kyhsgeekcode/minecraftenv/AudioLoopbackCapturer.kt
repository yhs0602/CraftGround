...
object AudioLoopbackCapturer {
    ...
    fun initialize(mainDevice: Long): Boolean {
        ...
        try {
            ...
            isInitialized = true
            printWithTime(
                "$TAG: Initialization complete - Sample Rate: $SAMPLE_RATE Hz, Channels: $CHANNELS, Buffer: ${BUFFER_SIZE_BYTES} bytes",
            )
     ...
     */
    fun getWaveformData(): ByteString = ByteString.copyFrom(lastCapturedSamples)

    /**
     * Get the last captured waveform data as a raw byte array.
     */
    fun getWaveformBytes(): ByteArray = lastCapturedSamples
