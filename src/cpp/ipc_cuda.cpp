...
DLManagedTensor* mtl_tensor_from_cuda_ipc_handle(void *cuda_ipc_handle, int width, int height) {
    ...
    cudaError_t err = cudaIpcOpenMemHandle(&device_ptr, *reinterpret_cast<cudaIpcMemHandle_t*>(cuda_ipc_handle),  cudaIpcMemLazyEnablePeerAccess);

    if (err != cudaSuccess) {
        throw std::runtime_error("Failed to open CUDA IPC handle: " + std::string(cudaGetErrorString(err)));
    }
    ...
    if (err != cudaSuccess) {
        throw std::runtime_error("Failed to get CUDA pointer attributes: " + std::string(cudaGetErrorString(err)));
    }
