...
// mc262 fork of shared-native/gl-capture/rgb_capture.cpp (docs/26_2_phase2_plan.md W3). 26.2
// replaced the old Framebuffer/raw-FBO-int model with GpuTexture/RenderTarget, so there is no
// FBO integer to hand over from the Java side anymore. Instead this file owns a single capture
// FBO and attaches whatever color texture id it's given as GL_COLOR_ATTACHMENT0, only
// re-attaching when the texture id actually changes (e.g. a window resize creates a new backing
// texture in Minecraft.renderFrame's RenderTarget). The exported signature (int, int, int) is
// unchanged from mc121's so rgb_capture.h didn't need to change - only the meaning of the first
// argument did (texture id instead of framebuffer id).
